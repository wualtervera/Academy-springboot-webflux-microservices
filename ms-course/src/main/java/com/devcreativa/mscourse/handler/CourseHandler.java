...
public class CourseHandler implements IOperations {
    ...
    public Mono<ServerResponse> findAll(ServerRequest request) {
        logTracer.printLog("controller_init", "request", request.uri());

        Flux<Course> courses = this.service.findAll();

        return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
            .body(courses, Course.class)
            .doOnNext(serverResponse -> {
                logTracer.printLog("controller_end", "response", serverResponse.statusCode());
            });
    }
